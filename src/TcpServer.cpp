...
TcpServer::TcpServer(const std::string& ip, const uint16_t port, int threadnum)
  ...
  for (int ii = 0; ii < threadnum_; ii++) {
    ...
    threadpool_.addtask(
      std::bind(&EventLoop::run, subloops_[ii].get())
    ); // 在线程池中运行从事件循环。
  }
}
...
void TcpServer::newconnection(std::unique_ptr<Socket> clientsock) {
  ...
  spConnection conn(
    new Connection(subloops_[clientfd % threadnum_].get(), std::move(clientsock))
  );
...
void TcpServer::sendcomplete(spConnection conn) {
  ...
  if (sendcompletecb_)
    sendcompletecb_(conn); // 回调EchoServer::HandleSendComplete()。
}
